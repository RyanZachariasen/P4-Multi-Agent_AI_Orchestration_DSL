...
let rec statement (stmt: py_statement) (output_file: out_channel) (indent: string) = 
  ...
  | PyIf (condition, body, orelse) -> 
    let rec emit_orelse else_branch =
      match else_branch with
      | [] -> ()
      | [PyIf (elif_condition, elif_body, elif_orelse)] ->
        output_string output_file (indent);
        output_string output_file ("elif ");
        Codegen_expr.expression elif_condition output_file;
        output_string output_file (":\n");
        List.iter (fun stmt -> statement stmt output_file (indent ^ "\t")) elif_body;
        emit_orelse elif_orelse
      | _ ->
        output_string output_file (indent);
        output_string output_file ("else:\n");
        List.iter (fun stmt -> statement stmt output_file (indent ^ "\t")) else_branch
    in
    output_string output_file (indent);
    output_string output_file ("if ");
    Codegen_expr.expression condition output_file;
    output_string output_file (":\n");
    List.iter (fun stmt -> statement stmt output_file (indent ^ "\t")) body;
    emit_orelse orelse;
